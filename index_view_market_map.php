...
<div class="row justify-content-center mt-1 mb-1 mx-0">
    ...
    <div class="row justify-content-center rounded">
        <div class="col-md-10 mb-3 pb-5 align-items-center justify-content-center bg-light text-dark rounded">
            <?php
            $sql = "SELECT * FROM stall, market_admin, map WHERE stall_status='2' AND map.market_ID='$market_ID' AND stall.map_ID=map.map_ID";
            ...
            <div class="row m-auto">
                <?php
                $map = "SELECT * FROM map WHERE map.market_ID='$market_ID'";
                $map1 = mysqli_query($conn, $map);
                while ($row1 = mysqli_fetch_assoc($map1)) {
                ?>
                    <div class="mt-4">
                        <h4 class="text-dark text-center">
                            <?= $row1['map_name'] ?> - Floor <?= $row1['map_floor'] ?>
                        </h4>
                    </div>
                    <div class="col-md-10 border border-secondary rounded shadow m-auto mt-3 p-0" id="map" style="position: relative;">
                        ...
                        <?php endwhile; ?>
                        <?php
                        $file_path = $row1['map_img'];
                        $newpath = str_replace("../", "", $file_path);
                        ?>
                        <img src="<?php echo $newpath; ?>" class="fit-image img-fluid" style="pointer-events: none;" id="mapimage" alt="market map" />
                    </div>
                <?php
                }
                ?>
